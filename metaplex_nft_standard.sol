...
import "metaplex/metadata/src/metadata.sol";

contract MyNFT is Token, Metadata {
  ...
  // The uri function returns the URI for the NFT metadata
  function uri(uint256 tokenId) public view returns (string memory uri) {
    // Get the metadata for the specified token
    NFTMetadata memory metadata = _getMetadata(tokenId);
